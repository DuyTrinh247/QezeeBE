...
export interface QuizAttemptEvent {
  id: string;
  attempt_id: string;
  event_type: 'start' | 'answer' | 'pause' | 'resume' | 'submit' | 'timeout' | 'abandon';
...
export interface CreateQuizAttemptEventData {
  attempt_id: string;
  event_type: 'start' | 'answer' | 'pause' | 'resume' | 'submit' | 'timeout' | 'abandon';
  event_timestamp: Date;
  event_data?: any;
...
export interface QuizAttemptEventFilter {
  attempt_id?: string;
  event_type?: string;
  question_id?: string;
  start_date?: Date;
...
export class QuizAttemptEventsRepository {
  private pool: Pool;
  ...
  async create(data: CreateQuizAttemptEventData): Promise<QuizAttemptEvent> {
  ...
  async findByAttemptId(attemptId: string, limit?: number, offset?: number): Promise<QuizAttemptEvent[]> {
    let query = `
  ...
  async findByFilter(filter: QuizAttemptEventFilter): Promise<QuizAttemptEvent[]> {
    const conditions = [];
    const values = [];
    let paramCount = 1;
  ...
  async findById(id: string): Promise<QuizAttemptEvent | null> {
  ...
  async getEventStats(attemptId: string): Promise<{
    total_events: number;
  ...
  async getTimeline(attemptId: string): Promise<Array<{
    timestamp: Date;
    event_type: string;
    question_number?: number;
    time_spent_ms?: number;
